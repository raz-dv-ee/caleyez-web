...
(function (root, factory) {
  ...
  // Daily calorie goal = TDEE shifted by intent. Floors at 1000 so a goal is never absurd.
  var GOAL_OFFSET = { lose: -500, maintain: 0, gain: 400 };
  function calorieTarget(p) {
    var t = tdee(p);
    if (t == null) return null;
    var off = GOAL_OFFSET[p && p.goal] || 0;   // unset/unknown goal behaves as maintain
    return Math.max(1000, t + off);
  }

  // Macro split of a calorie target: 30% protein, 40% carbs, 30% fat (protein/carbs 4 kcal/g, fat 9 kcal/g).
  function macroTargets(target) {
    if (!target) return null;
    return {
      pro: Math.round(target * 0.30 / 4),
      carb: Math.round(target * 0.40 / 4),
  ...
  return {
    ...
    bmi: bmi, bmr: bmr, tdee: tdee, calorieTarget: calorieTarget, macroTargets: macroTargets,
